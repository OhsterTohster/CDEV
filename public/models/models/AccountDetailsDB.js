...
class AccountDetailsDB {
    ...
    userLogin(request, respond) {
        ...
        var sql = "SELECT account_details.activated = 1 AS activated , account_details.email, password.password FROM project.account_details INNER JOIN project.password ON account_details.userid = password.userid WHERE email = ? ";
    ...
    reactivateUser(request, respond) {
        ...
        var input = request.body.input;
        var nodemailer = require('nodemailer');
        var verifymsg = "Reactivation Code has been sent to your email address."
        var transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: 'help.niceanot@gmail.com',
    ...
    editUserInfo(request, respond) {
        var msg = " ";
        var userObject = new AccountDetails(request.body.username, request.body.userid, request.body.email, request.body.gender, request.body.full_name, request.body.mobile_number, request.body.address_line_1, request.body.address_line_2, request.body.postal_code, null, null, request.body.profile_url);
        var values = [userObject.getUsername(), userObject.getEmail(), userObject.getGender(), userObject.getFullName(),
            userObject.getMobileNumber(), userObject.getAddress1(), userObject.getAddress2(), userObject.getPostalCode(), userObject.getProfileURL(), userObject.getUserId()];
        var sql = "UPDATE project.account_details SET username = ?, email = ?, gender = ?, full_name = ?, mobile_number = ?, address_line_1 = ?, address_line_2 = ?, postal_code = ?, profile_url = ? WHERE userid = ?";
        db.query(sql, values, function(error, result){
            ...
            if(error) {
                ...
                var password = request.body.password;
                var bruh = [password, userid];
                var changepass = "UPDATE project.password SET password = ? WHERE userid = ?"
                db.query(changepass, bruh, function(error,result){
                    console.log(userid);
                    ...
                    if(error) {
                        ...
                        msg = "Account Details successfully updated"
                        console.log(result);
                        respond.json(msg);
                    }
                })
